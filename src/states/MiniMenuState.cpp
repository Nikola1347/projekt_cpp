...
// KONSTRUKTOR - TWORZY OPCJE MENU NA PODSTAWIE STANU POLA
MiniMenuState::MiniMenuState(
    ...
    : fieldID(id),
      ...
      onHarvest(onHarvest)
{
    // wybór opcji zależnie od stanu pola
    switch (fieldState)
    {
...
void MiniMenuState::setNoSeedsMessage()
{
    if (!options.empty() && options[0] == "Zasadz")
    {
        options[0] = "Brak nasion";
        texts[0].setString("Brak nasion");
    }
}
...
void MiniMenuState::draw(Game& game) {
    ...
    {
        boxes[i].setPosition(box.getPosition().x + 20.f, box.getPosition().y + titleHeight + i * optionHeight);
